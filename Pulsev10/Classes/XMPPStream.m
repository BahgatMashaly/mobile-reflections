...
- (void)sendElement:(NSXMLElement *)element withTag:(long)tag
{
	...
	else
	{
		...
		{
//			if ([[[element childElement] name] isEqualToString:@"x"]) {
//				
//			}
//			else {
...
- (void)tryNextSrvResult
{
	...
	if (!success)
	{
		...
		success = [self connectToHost:[myJID domain] onPort:PORTNUMBER error:&connectError];
	}
...
- (void)xmppParser:(XMPPParser *)sender didReadElement:(NSXMLElement *)element
{
	...
	else
	{
		if([elementName isEqualToString:@"iq"])
		{
			...
			{
                /* Commented out to prevent bombardment -John
